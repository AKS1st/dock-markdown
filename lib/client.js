...
Please report this to https://github.com/markedjs/marked.`, e) {
		...
		*/
		const INLINE = {
			...
			toolbarBtnOn: {
				border: "1px solid var(--dsw-alias-interactive-fg-accent, #3b6fe0)",
				background: "var(--dsw-alias-interactive-bg-hover, rgba(0,0,0,0.06))",
				color: "var(--dsw-alias-interactive-fg-accent, #3b6fe0)",
				borderRadius: 5,
				padding: "2px 9px",
				cursor: "pointer",
				fontSize: 13,
			...
			outline: {
				...
				flexShrink: 0,
				overflow: "auto",
				borderRight: "1px solid var(--dsw-alias-border-l2, #d8dbe0)",
...
`;
		...
		*  can scroll (marked v16 no longer emits heading ids). */
		...
		const raf = typeof requestAnimationFrame === "function" ? requestAnimationFrame.bind(globalThis) : ((callback) => {
			return setTimeout(() => callback(0), 16);
		});
		const caf = typeof cancelAnimationFrame === "function" ? cancelAnimationFrame.bind(globalThis) : ((handle) => {
			clearTimeout(handle);
		});
		function MarkdownView(props) {
			...
			const [binary, setBinary] = (0, react.useState)(false);
			const [truncated, setTruncated] = (0, react.useState)(false);
			const [error, setError] = (0, react.useState)(null);
			const [html, setHtml] = (0, react.useState)(null);
			/** Sanitized html with resolved srcs / link attributes substituted in. */
			const [displayHtml, setDisplayHtml] = (0, react.useState)(null);
			/** ref → resolution state (drives the display rebuild). */
			const [resolutions, setResolutions] = (0, react.useState)({});
			/** Document outline (headings h1–h6) of the current document. */
			const [outline, setOutline] = (0, react.useState)([]);
			/** Whether the outline rail is visible. */
			const [outlineOpen, setOutlineOpen] = (0, react.useState)(false);
			/** The heading currently in view (scroll-spy highlight). */
			const [activeHeading, setActiveHeading] = (0, react.useState)(null);
			...
			const docKeyRef = (0, react.useRef)("");
			/** The html the current outline was collected from (collect once per doc). */
			const outlineHtmlRef = (0, react.useRef)(null);
			...
			(0, react.useEffect)(() => {
				...
				(async () => {
					try {
						const json = await (await fetch("/desk-editor/fs.read", {
							method: "POST",
			...
			(0, react.useEffect)(() => {
				...
				try {
					const raw = k.parse(content, {
						async: false,
						gfm: true,
						breaks: true
					});
					setHtml(purify.sanitize(raw));
				} catch (cause) {
					setError(cause instanceof Error ? cause.message : String(cause));
					setHtml(null);
				}
			...
			(0, react.useEffect)(() => {
				...
				for (const ref of refs) {
					...
					(async () => {
						...
						try {
							const json = await (await fetch("/dock-markdown/resolve", {
								method: "POST",
								headers: { "content-type": "application/json" },
								body: JSON.stringify({
									sessionId,
							...
							if (json.ok !== true || json.value === void 0) throw new Error(json.error?.message ?? "resolve failed");
							const value = json.value;
							if (!value.found) apply({ status: "missing" });
							else if (value.kind === "image" && value.dataUrl !== void 0) apply({
								status: "ok",
								kind: "image",
							...
							else if (value.kind === "image" && value.tooLarge === true) apply({
								status: "ok",
								kind: "image",
								tooLarge: true,
								size: value.size ?? 0
							});
							else if (value.kind === "link" && value.path !== void 0) apply({
								status: "ok",
								kind: "link",
								path: value.path
							});
							else apply({ status: "missing" });
						} catch {
							if (docKeyRef.current !== docKey) return;
				...
				for (const img of doc.querySelectorAll("img[src]")) {
					...
					if (resolution?.status === "ok" && resolution.kind === "image" && resolution.dataUrl !== void 0) img.setAttribute("src", resolution.dataUrl);
					else img.setAttribute("src", PLACEHOLDER_SRC);
					if (resolution?.status === "ok" && resolution.kind === "image" && resolution.tooLarge === true) img.setAttribute("title", `image too large (${resolution.size} bytes, over the 20 MiB limit)`);
				}
				for (const anchor of doc.querySelectorAll("a[href]")) {
					const href = anchor.getAttribute("href");
					...
					if (resolution?.status === "ok" && resolution.kind === "link") anchor.setAttribute("data-md-path", resolution.path);
				}
			...
			const scrollToHeading = (id) => {
				...
				if (heading !== void 0 && heading !== null && typeof heading.scrollIntoView === "function") heading.scrollIntoView({
					behavior: "smooth",
					block: "start"
				});
			};
			(0, react.useEffect)(() => {
				...
				const update = () => {
					...
					for (const item of outline) {
						...
						if (heading instanceof HTMLElement && heading.getBoundingClientRect().top - containerTop <= 12) current = item.id;
					}
			...
			const switchToEditor = () => {
				...
				ctx.get("workbench")?.openView("editor", {
					path,
					title
				}, { floating: true });
			};
			...
			*  scroll to the heading, and unresolved relative links never navigate. */
			const onMarkdownClick = (event) => {
				...
				if (fileLink instanceof HTMLAnchorElement) {
					...
					if (targetPath !== null) ctx.get("workbench")?.openPath(targetPath);
					return;
				}
				if (target.closest("a[data-md-rel]") !== null) event.preventDefault();
			};
			const title = openSeed.title ?? path?.split("/").pop() ?? "No file";
			return (0, react.createElement)("div", { style: INLINE.wrap }, (0, react.createElement)("style", { dangerouslySetInnerHTML: { __html: MD_CSS } }), (0, react.createElement)("div", { style: INLINE.head }, (0, react.createElement)("button", {
				...
				style: INLINE.title,
				title: path
			}, title), truncated ? (0, react.createElement)("span", { style: INLINE.empty }, t("fileTooLarge")) : null), error !== null ? (0, react.createElement)("div", { style: INLINE.err }, error) : content === null ? (0, react.createElement)("div", { style: INLINE.empty }, "Reading…") : binary ? (0, react.createElement)("div", { style: INLINE.err }, "Binary file — markdown preview is not supported.") : (0, react.createElement)("div", { style: INLINE.body }, outlineOpen ? (0, react.createElement)("div", { style: INLINE.outline }, (0, react.createElement)("div", { style: INLINE.outlineTitle }, t("outline")), outline.length === 0 ? (0, react.createElement)("div", { style: INLINE.outlineEmpty }, t("noHeadingStructure")) : outline.map((item) => (0, react.createElement)("button", {
				key: item.id,
				style: {
					...INLINE.outlineItem,
			...
			}, item.text === "" ? t("untitled") : item.text))) : null, (0, react.createElement)("div", {
				style: INLINE.scroll,
				ref: scrollRef,
				onClick: onMarkdownClick
			}, (0, react.createElement)("div", { dangerouslySetInnerHTML: { __html: displayHtml ?? "" } }))));
		}
		//#endregion
